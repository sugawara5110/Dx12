...
bool StateMenu::TitleMenu(Directionkey direction) {
	...
	text->UpDateText(L"ＮＥＷＧＡＭＥ", 320.0f, 250.0f, 30.0f, { r1, g1, 1.0f, 1.0f });
	text->UpDateText(L"ＣＯＮＴＩＮＵＥ", 320.0f, 290.0f, 30.0f, { r2, g2, 1.0f, 1.0f });
	return TRUE;
}
...
bool StateMenu::Main_Menu(Position::H_Pos *h_p, int map_no, int *boss_killed, Hero *hero, Directionkey direction) {
	...
	Menucreate();
	for (int i = 0; i < 4; i++) {
		text->UpDateText(L"囚人Ｎｏ", 160.0f, 10.0f + 110.0f * i, fsize, { 1.0f, 1.0f, 1.0f, 1.0f });
		text->UpDateValue(i, 270.0f, 10.0f + 110.0f * i, fsize, 1, { 1.0f, 1.0f, 1.0f, 1.0f });
		text->UpDateText(L"ＨＰ", 160.0f, 40.0f + 110.0f * i, fsize, { 1.0f, 1.0f, 1.0f, 1.0f });
		...
		text->UpDateText(L"ＭＰ", 160.0f, 70.0f + 110.0f * i, fsize, { 1.0f, 1.0f, 1.0f, 1.0f });
	...
	fsize = 15.0f;
	text->UpDateText(L"ステータス", 10, 10, fsize, { r1, g1, 1.0f, 1.0f });
	text->UpDateText(L"回復", 10, 40, fsize, { r2, g2, 1.0f, 1.0f });
	text->UpDateText(L"ＰＰ振り分け", 10, 70, fsize, { r3, g3, 1.0f, 1.0f });
	text->UpDateText(L"セーブ", 10, 100, fsize, { r4, g4, 1.0f, 1.0f });
...
void StateMenu::St_Menu(Hero *hero, Directionkey direction) {
	...
	float fsize = 25.0f;
	St_create();
	text->UpDateText(L"囚人Ｎｏ", 120.0f, 10.0f, fsize, { 1.0f, 1.0f, 1.0f, 1.0f });
	text->UpDateValue(st_select, 230.0f, 10.0f, fsize, 1, { 1.0f, 1.0f, 1.0f, 1.0f });
	text->UpDateText(L"ＬＶ", 120.0f, 40.0f, fsize, { 1.0f, 1.0f, 1.0f, 1.0f });
	...
	if (hero[st_select].GetHealing_LV() != 0) {
		text->UpDateText(L"ヒーリングＬＶ", 120.0f, 280.0f, fsize, { 1.0f, 1.0f, 1.0f, 1.0f });
	...
	if (hero[st_select].GetRecover_LV() != 0) {
		text->UpDateText(L"リカバリＬＶ", 120.0f, 340.0f, fsize, { 1.0f, 1.0f, 1.0f, 1.0f });
...
void StateMenu::Re_Menu(Hero *hero, Directionkey direction) {
	...
	for (int i = 0; i < 4; i++) {
		text->UpDateText(L"囚人Ｎｏ", 10.0f, 10.0f + 20.0f * i, fsize0, { nr[i], ng[i], 1.0f, 1.0f });
		text->UpDateValue(i, 75.0f, 10.0f + 20.0f * i, fsize0, 1, { nr[i], ng[i], 1.0f, 1.0f });
		text->UpDateText(L"ＨＰ", 160.0f, 40.0f + 110.0f * i, fsize, { r[i], g[i], 1.0f, 1.0f });
	...
	if (re_select1 != -1) {
		Re_create1();
		if (hero[re_select].GetHealing_LV() >= 1) {
			text->UpDateText(L"ヒーリング", 10.0f, 160.0f, fsize0, { mr[0], mg[0], 1.0f, 1.0f });
		}
		if (hero[re_select].GetRecover_LV() >= 1) {
			text->UpDateText(L"リカバリ", 10.0f, 190.0f, fsize0, { mr[1], mg[1], 1.0f, 1.0f });
		}
	}
}
