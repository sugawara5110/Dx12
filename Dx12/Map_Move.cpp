...
Encount Map::Move(MapState *mapstate, Directionkey direction) {
	...
	//当たり判定
	int ind = posz * mxy.y * mxy.x + (int)(rounding2((int)cay1Tmp, 2, 1, 9) * 0.01f) * mxy.x + (int)(rounding2((int)cax1Tmp, 2, 1, 9) * 0.01f);
	if (direction == UP) {
		if (MoveUpCond(ind))return NOENCOUNT;
	}
	if (direction == DOWN) {
		if (MoveDownCond(ind))return NOENCOUNT;
	}
	//座標更新
	cax1 = cax1Tmp;
	cax2 = cax2Tmp;
